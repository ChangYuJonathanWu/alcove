...
const getPublicProfile = async (handle, uid) => {
    try {
        const getClientPromiseTimerName = `getPublicProfile: await clientPromise: ${handle}_${uid}`
        console.time(getClientPromiseTimerName)
        const client = await clientPromise
        console.timeEnd(getClientPromiseTimerName)
        const db = client.db("home");
        const profiles = db.collection("profiles")
        const query = { $or: [{ handle }, { uid }] }

        const profileQueryTimerName = `getPublicProfile: Database Query: ${handle}_${uid}`
        console.time(profileQueryTimerName)
        const result = await profiles.findOne(query)
        console.timeEnd(profileQueryTimerName)
        if (result) {
            result["email"] = null;
            result["_id"] = null;
        }
...
const addPostToList = async (post) => {
    const {
        postType, spotifyUri, instagramUri, youtubeUri, title, subtitle, caption, listId, photo, uri, uid
    } = post
    try {
        console.log(`Reordering profile items for ${uid}`)
        const client = await clientPromise
        ...
        if (postType === "standard") {
            ...
            const parsedUri = new URL(youtubeUri)
            const youtubeId = parsedUri.searchParams.get("v")
            newItem = {
                id: itemId,
                type: postType,
                parentId: listId,
