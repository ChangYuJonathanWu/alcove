...
const renameProfileItem = async (itemId, newName, subtitle, uid) => {
    try {
        const client = await clientPromise
        const db = client.db("home")
        const profiles = db.collection("profiles")
...
const addPostToList = async(title, subtitle, caption, listId, photo, uid) => {
    try {
        console.log(`Reordering profile items for ${uid}`)
        const client = await clientPromise
        const db = client.db("home")
...
module.exports = { getPublicProfile, getFullProfile, updateProfile, addProfileItem, deleteProfileItem, renameProfileItem, rearrangeProfileItems, addPostToList, deletePost, editPost, reorderPosts }